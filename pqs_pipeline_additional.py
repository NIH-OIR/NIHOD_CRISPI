...
def get_pqs_protocol_detail_response(session, conf_file, protocol_number):
    protocol_detail_url = conf_file['PQS_API']['pqs_protocol_detail_testurl'] + protocol_number
    # print(protocol_detail_url)
    response = session.get(protocol_detail_url, verify=False)
    return response

def get_pqs_protocol_detail_additional_response(session, conf_file, protocol_number):
    protocol_detail_url = conf_file['PQS_API']['pqs_protocol_detail_additional_url'] + protocol_number
    # print(protocol_detail_url)
    response = session.get(protocol_detail_url, verify=False)
    return response
...
def generate_pqs_tables(conf_file, conn, engine):
    ...
    protocol_data_fields = ['protocol_number', 'simpleProtocolNumber', 'protocol_title', 'accrual_inst','accrual_status','coord_site','protrak_accrual_status','research_type','research_phase', 'study_type',
               ...
               'initialRegistrationToCTGov','needsToComplyWithFDAAA','reportingResultsRequired','reportingInstitute','resultsEnteredReleasedQA','dateResultsFirstReleased',
               'dateResultsLastReleased','resultsPublishedCTGov','resultsPublishedWithinYear']
    ...
    for protocol in protocol_list:   
        ...
        protocol_detail_response = get_pqs_protocol_detail_additional_response(protocol_detail_session, conf_file, protocol_number)
        try:
            if protocol_detail_response.status_code != 200 or protocol_detail_response.json()['returnedProtocol'] is None:
                time.sleep(.5)
                continue
        except:
        ...
        flattened_detail_json['initialRegistrationToCTGov'] = detail_json.get('initialRegistrationToCTGov','')
        flattened_detail_json['needsToComplyWithFDAAA'] = detail_json.get('needsToComplyWithFDAAA','')
        flattened_detail_json['reportingResultsRequired'] = detail_json.get('reportingResultsRequired','')
        flattened_detail_json['reportingInstitute'] = detail_json.get('reportingInstitute','')
        flattened_detail_json['resultsEnteredReleasedQA'] = detail_json.get('resultsEnteredReleasedQA','')
        flattened_detail_json['dateResultsFirstReleased'] = detail_json.get('dateResultsFirstReleased','')
        flattened_detail_json['dateResultsLastReleased'] = detail_json.get('dateResultsLastReleased','')
        flattened_detail_json['resultsPublishedCTGov'] = detail_json.get('resultsPublishedCTGov','')
        flattened_detail_json['resultsPublishedWithinYear'] = detail_json.get('resultsPublishedWithinYear','')
    ...
    final_table.index += 1
    final_table.to_csv(f"./table_backup/Protrak_Data_Table_Additional_{date}.csv")

    table_name = 'Protrak_Data_Table_Additional'
    ...
    final_table.to_sql(table_name, con=engine.connect(), if_exists='replace', index=True, index_label='id', method='multi')
...
if __name__ == '__main__': 
    ...
    generate_pqs_tables(conf_file, conn, engine)
